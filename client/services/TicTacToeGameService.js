...
.factory('Game',function(gridSize, Board, Player){
  var Game = function(gridSize){
    this.board = new Board.board(gridSize);
    this.human = new Player('O');
    this.computer = new Player('X');
  };

  Game.prototype = {
    humanMove : function(row, column){
      var completedTurn = this.board.placePiece(this.human.get('symbol'),row,column);
      if(completedTurn){
        this.computerMove();
      }
    },
    computerMove : function (){
      //comp ai goes here which returns a locatoin
      this.board.placePiece(this.computerPlayer.get('symbol'),index);
    },
    newBoard : function(){
